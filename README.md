...
The examples here are assuming that the .bam files were generated by using short-read RNA-seq data from TCGA. If you used STAR to generate the .bam files, you should have .SJ.out.tab files already. In the groupa.txt and groupb.txt files, please use the suffix of the .SJ.out.tab files. For example, the suffix of a Sequins_MixA.SJ.out.tab file is "Sequins_MixA".
```
